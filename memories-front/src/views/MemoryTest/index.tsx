...
function Card({ memoryCard, onClick }: CardProps) {
    ...
    if (isReverse) return <div className="reversed-card" onClick={() => onClick(id)}></div>;
...
export default function MemoryTest() {
    ...
    const onCardClickHandler = (id: number) => {
        ...
        const newMemoryCards: MemoryCard[] = memoryCards.map((card) =>
            card.id === id ? { ...card, isReverse: false } : card
        );
            ...
            setTimeout(() => {
                if (first.color !== second.color) {
                    const newMemoryCards = memoryCards.map((card) =>
                        card.id === first.id || card.id === second.id ? { ...card, isReverse: true } : card
                    );
                    setMemoryCards(newMemoryCards);
                }
                setSelectedCards([]);
    ...
    useEffect(() => {
        ...
        const initMemoryCards: MemoryCard[] = MIX_COLORS.map((color, id) => ({
            id,
            color,
            isReverse: false,
        }));
        ...
        setTimeout(() => {
            const memoryCards = initMemoryCards.map((card) => ({ ...card, isReverse: true }));
            ...
            <div className="container">
                ...
                <div className="test-box">
                    {isStarted ? (
                        ...
                        <div className="button middle primary" onClick={onStartClickHandler}>
                            검사 시작
                        </div>
                    )}
                </div>
